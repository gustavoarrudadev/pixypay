...
/**
 * Página especial para visualização como revenda (impersonation)
 * Acessível apenas via token gerado por admin
 * Usa localStorage para modo de impersonation temporário
 */
export default function VisualizarRevendaPage() {
  ...
  const processarToken = async () => {
    try {
      ...
      localStorage.setItem('impersonation_mode', JSON.stringify({
        revendaUserId: dadosToken.revendaUserId,
        revendaId: dadosToken.revendaId,
        adminUserId: dadosToken.adminUserId,
        timestamp: dadosToken.timestamp,
      }))

      // Redireciona para a área da revenda
    ...
    <div className="min-h-screen flex items-center justify-center bg-neutral-50 dark:bg-neutral-900">
      <div className="text-center">
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400">
          Carregando visualização como revenda...
        </p>
      </div>
    </div>
  )
